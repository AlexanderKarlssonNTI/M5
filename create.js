...
const generateWorld = function (showAsWell = true) {
    ...
    switch (wantedWorldType) {
        case 'circle':
            createdWorld = new World('','circle', parseInt(numberOfRoomsConfig.value),0);
            break;
        case 'rectangle':
            createdWorld = new World('','rectangle', parseInt(lengthConfig.value),parseInt(widthConfig.value));
            break;
        case 'branch':
            createdWorld = new World('','branch', parseInt(startLengthConfig.value),parseFloat(branchFactorConfig.value));
            break;
    }
...
function saveSpecifiedWorld() {
    ...
    if (currentWorld == null) {
        // No preview and failed to genreate new world:
        console.error('Please make a world first')
        return;
    }
    let data = new FormData();
    data.append("type" , wantedWorldType);
